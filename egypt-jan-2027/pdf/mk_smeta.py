...
rows = [
 ...
 ("КОМАНДА LA ROYAL", "=Команда!$B$21", "за группу",
  "=B{row}*Команда!$B$23/"+G, "=B{row}*Команда!$B$23/"+G,
...
ws.cell(r, 1, "ОПЦИИ СВЕРХ ПРОГРАММЫ — оплачиваются отдельно, в цену выше не входят").font = Font(bold=True, color="6E8F13")
r += 1
for name, price, note in [
    ("Абу-Симбел самолётом, на человека", 470, "Летят не все. Билеты и транспорт на месте — цены пока нет"),
    ("Абу-Симбел: билеты и транспорт на месте", 0, "ЦЕНЫ НЕТ"),
]:
    ws.cell(r, 1, name).alignment = WRAP
    c = ws.cell(r, 2, price); c.fill = INP; c.border = BOX; c.number_format = MON
    n = ws.cell(r, 6, note); n.alignment = WRAP; n.font = I
...
for col in (4, 5):
    L = chr(64 + col)
    c = ws.cell(r, col, f"={L}{marg}*{G}-Команда!$B$21*(1-Команда!$B$23)")
...
tp = [("Человек в команде",            4, "Всего в поездке"),
      ...
      ("Едет на выезды",               2, "Рестораны и опции — не весь состав"),
...
switch_row = rr
assert total_row == 21 and switch_row == 23, (total_row, switch_row)
rr += 2
