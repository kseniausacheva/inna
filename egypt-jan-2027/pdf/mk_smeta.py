...
rows = [
 ...
 ("Ужин на острове Филе", 150, "на гостя",
  "=B{row}", "=B{row}",
  "Названо клиентом. Сверх 15 000 EUR за приватный вечер"),
 ("Завтрак в Khufu’s", 90, "на гостя",
  "=B{row}", "=B{row}",
  "Названо клиентом"),
 ("Большой Египетский музей", 120, "на гостя",
  "=B{row}", "=B{row}",
  "Индивидуальная экскурсия с трансфером. Названо клиентом"),
 ("Гиза: приватный доступ, одно посещение", 3500, "за посещение",
  "=B{row}*2/"+G, "=B{row}*2/"+G,
  "ПОДТВЕРЖДЕНО: приватное открытие для группы. Билеты НЕ входят, см. строку ниже"),
 ("Входной билет на плато Гизы", 180, "за билет",
  "=B{row}*2", "=B{row}*2",
  "180 за билет. ПРОВЕРИТЬ: один билет на оба посещения или два"),
 ...
 ("Отель 29 января: Асуан (A) / Four Seasons 30-го (B)", 250, "на гостя",
  "=B{row}", "=570",
  "A: остров Элефантина, открытые источники 140–330. B: Four Seasons, 570 названо отелем"),
 ...
 ("Перелёты Каир–Луксор и Асуан–Каир", 250, "на гостя",
  "=B{row}", "=B{row}",
  "ОЦЕНКА по открытым источникам: 140 + 110. Групповой тариф запрошен"),
 ("Прочее: meet & greet, fast track", 0, "на гостя",
  "=B{row}", "=B{row}",
  "ЦЕНЫ НЕТ"),
...
tp = [("Человек в команде",            4, "Всего в поездке"),
      ...
      ("Отель 29-го, за номер",      570, "570 = Four Seasons (B). Для варианта A ставим 250"),
...
lines = [
 ...
 ("Отель 29-го: Асуан 250 (A) или FS 570 (B)", f"={SR}*{FSN}", "Номеров × цена. Для A поставить 250"),
 ("Абу-Симбел самолётом", f"={OUT}*470", "Летят не все"),
 ("Ужины вне борта: Lucida, Филе, Khufu, прощальный", f"={OUT}*760+{OUT}*360", "рестораны 760 + билеты в Гизу 360"),
 ("Питание на берегу", f"={S}*{TF}", "Весь состав"),
 ("Внутренние перелёты команды", f"={S}*250", "Те же 250 на человека"),
]
