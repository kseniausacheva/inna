...
rows = [
 ...
 ("Наземная программа: экскурсии, гиды, доступы в храмы", 750, "на гостя",
  "=B{row}", "=B{row}",
  "НАЗВАНО 22.09. Состав уточняется: возможны пересечения с фрахтом, билетами в Гизу и GEM"),
 ("Giza Palace, ночь", 325, "за номер за ночь",
  "=B{row}*2", "=B{row}*2",
  "Две ночи. Открытые источники 222–349. Запросить групповой тариф"),
 ("Sonesta St. George Luxor, ночь", 222, "за номер за ночь",
  "=B{row}", "=B{row}",
  "ПИСЬМО ОТЕЛЯ 22.09: 18 Royal Elite, single BB, сервис и НДС включены. Ужин отдельно"),
 ("Отель 29 января: Асуан (A) / Four Seasons 30-го (B)", 250, "на гостя",
  "=B{row}", "=550",
  "A: остров Элефантина, открытые источники 140–330. B: Four Seasons, 550 названо отелем"),
 ("Прощальный ужин в Four Seasons", 250, "на гостя",
  "=B{row}", "=0",
  "Net или ++? С надбавками 319. Только вариант A"),
 ("Ужин в Lucida", 150, "на гостя",
  "=B{row}", "=B{row}",
  "ПОДТВЕРЖДЕНО: 150, два бокала вина или пива включены"),
...
tp = [("Человек в команде",            4, "Всего в поездке"),
      ...
      ("Отель 29-го, за номер",      550, "550 = Four Seasons (B). Для варианта A ставим 250"),
...
lines = [
 ...
 ("Отель 29-го: Асуан 250 (A) или FS 550 (B)", f"={SR}*{FSN}", "Номеров × цена. Для A поставить 250"),
...
ws2.cell(rr, 1, "ПЕРЕКЛЮЧАТЕЛЬ: 1 = в себестоимость, 0 = из вознаграждения. РЕШЕНО: 0").font = Font(bold=True, color="5B46B8")
c = ws2.cell(rr, 2, 0); c.fill = INP; c.border = BOX
...
ws3["A1"] = "ВОПРОСЫ, БЕЗ КОТОРЫХ СЧИТАТЬ НЕЛЬЗЯ"
...
qs = [
 ...
 ("Групповой блок номеров на январь: Giza Palace и Four Seasons",
  "Отделы продаж отелей", "325 — из интернета, ±30 %. По Луксору цена есть от отеля"),
 ("Подтвердить письменно: 30 кают, сколько продаются в одноместном, где живут экипаж и наша команда",
  "Судовой оператор", "От этого потолок группы: 27 гостей при одноместном размещении"),
 ("Блоки номеров и места на рейсах под финальное число гостей, а не под 16",
  "Отели и авиакомпании", "Sonesta дала 18 номеров, на 27 гостей нужно 30. Рейсы в январе кончаются"),
 ("Сколько корзин шара дают на одну дату и поднимают ли их одновременно?",
  "Оператор шаров, Луксор", "После 24 гостей нужна четвёртая корзина, 2 500 каждая"),
 ("Меняется ли цена Филе и открытий в Гизе от размера группы?",
  "Мара / принимающая компания", "Считаем их постоянными — это 24 тыс. в постоянной части"),
]
rr = 4
